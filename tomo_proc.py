...
def find_drift(h5file, rowids=[0,-1], signalid=8, flip=False, shift=0):
    #assume first and last row of ims is 0 and 360 degrees.
    import h5py

    with h5py.File(h5file, 'r', locking=True) as h5f:
        ims = np.array(h5f['norm/channel00/ims'])
        pos0 = np.array(h5f['mot1'])[0,:]

    # overplotting the signal at 0 and 360 degrees. These should be identical (i.e. full rotation)
    plt.plot(pos0,ims[signalid,rowids[0],:])
    if flip is False:
        plt.plot(pos0+shift,ims[signalid,rowids[1],:])
    else:
        plt.plot(pos0+shift,np.flip(ims[signalid,rowids[1],:]))
    plt.show()
   

def find_cor(h5file, transshift=None, corrange=[150, 153, 0.01], signalid=8, bkgr=None):
    ...
    with h5py.File(h5file, 'r', locking=True) as h5f:
        ...
        rot = np.array(h5f['mot2']) #motor positions expected in degrees
        trans = np.array(h5f['mot1'])
        print(np.array(h5f['mot2'])[:,0])
 
        angle = rot[:,0]*np.pi /180 #motor positions expected in degrees, convert to rad
    ...
    if transshift is not None:
        ...
        print("Interpolating Translation shift positions...", end='')
        shift = np.arange(ims.shape[1])/(ims.shape[1]-1)*transshift
        ...
        for k in range(ims.shape[0]):
            values = ims[k,:,:].ravel()
            ims[k,:,:] = griddata((x, y), values, (mot1_tmp, mot2_tmp), method='cubic', rescale=True).T
        print('Done')
    ...
    proj = proj[:,signalid,:].reshape((proj.shape[0],1,proj.shape[2]))
    plt.imshow(proj[:, 0, :])
    plt.show()
...
def h5_tomo_proc(h5file, rot_mot=None, rot_centre=None, signal='Ba-K', datadir='norm', channel='channel00', ncol=8, selfabs=None, snake=False, interp_tr=False, limit_rotrange=None, transshift=None, bkgr=None):
    ...
    if transshift is not None:
        ...
        for k in range(ims.shape[0]):
            values = ims[k,:,:].ravel()
            ims[k,:,:] = griddata((x, y), values, (mot1_tmp, mot2_tmp), method='cubic', rescale=True).T
        i0 = griddata((x, y), i0.ravel(), (mot1_tmp, mot2_tmp), method='cubic', rescale=True).T
        i1 = griddata((x, y), i1.ravel(), (mot1_tmp, mot2_tmp), method='cubic', rescale=True).T
        print('Done')
...
def h5_i1tomo_recon(h5file, rot_mot=None, rot_centre=None, snake=False, channel='channel00', limit_rotrange=None, transshift=None):
    ...
    if i1 is not None:
        ...
        if transshift is not None:
            from scipy.interpolate import griddata
            print("Interpolating Translation shift positions...", end='')
            shift = np.arange(i0.shape[0])/(i0.shape[0]-1)*transshift
            for k in range(shift.shape[0]):
                mot1[k,:] += shift[k]
            mot1_pos = mot1[0,:]
            mot2_pos = mot2[:,0]
