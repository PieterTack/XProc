...
##############################################################################
def ConvMxrfSpe(speprefix, outfile, mot1_name='X', mot2_name='Y', scandim=None):
    """
    ...
    scandim : tuple, optional
        tuple containing the scan dimensions (X,Y). If none, dimensions will be derived from filename separators or 
        data will be written as 1D.
    ...
    for file in spefiles:
        temp = file.replace('\\','/').split(speprefix.replace('\\','/'))[-1].split('.')[0]
        if '_' in temp:
            temp = temp.split('_')
    ...
    for index, file in enumerate(sorted(spefiles)):
        ...
        if not readas1d:
            temp = file.replace('\\','/').split(speprefix.replace('\\','/'))[-1].split('.')[0].split('_')
            xid = int(temp[1])-1
            yid = int(temp[0])-1
        else:
            xid = index
    ...
    if scandim:
        spectra = spectra.reshape((scandim[0], scandim[1], sumspec.shape[0]))
        icr = np.asarray(icr).reshape((scandim[0], scandim[1]))
        ocr = np.asarray(ocr).reshape((scandim[0], scandim[1]))
        i0 = np.asarray(i0).reshape((scandim[0], scandim[1]))
        i1 = np.asarray(i1).reshape((scandim[0], scandim[1]))
    ...
    outfile = '/'.join(speprefix.replace('\\','/').split('/')[:-1])+'/'+outfile
    ...
    with h5py.File(outfile, 'w', locking=True) as f:
        ...
        f.create_dataset('raw/channel00/icr', data=np.squeeze(icr), compression='gzip', compression_opts=4)
